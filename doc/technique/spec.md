...
- **Base de données** : PostgreSQL en **service séparé** (image officielle `postgres:16-alpine`) + Prisma pour le schéma/les migrations. *Historique : embarqué dans l'image `web` un temps pour la simplicité, mais initdb à chaque premier démarrage rendait le boot du conteneur trop lent — séparé dès qu'observé en pratique.* Migrations versionnées appliquées par un conteneur `migrate` one-shot avant que `web`/`worker` ne démarrent (`prisma migrate deploy`, équivalent Flyway — jamais `migrate dev` en dehors du poste de dev).
...
- **Authentification : auth maison**, pas de lib tierce — next-auth v5 écarté après audit (beta depuis fin 2023, cadence de release en net ralentissement, même motif que l'abandon de Lucia). Session opaque stockée en DB (`Session.tokenHash`, cookie httpOnly, révocation triviale), + flux d'invitation par lien pour rejoindre sans que l'Admin crée chaque compte manuellement. **Validé bout en bout.** `[Q38]`
- **Déploiement** : un `docker-compose.yml` (`postgres`, `migrate` one-shot, `web`, `worker`, `redis`) + reverse proxy Caddy (TLS automatique) devant `web`. **Sauvegarde `pg_dump` planifiée dès la V1** — la conteneurisation ne remplace pas une sauvegarde, l'historique de campagne et les fiches perso n'ont pas de filet sinon. **Validé bout en bout** : build Docker + migrations + seed + auth (login/session/logout) + page lisant la DB au runtime fonctionnent, démarrage à ~5s (voir [architecture.md](architecture.md)).
