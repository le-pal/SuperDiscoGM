...
- **Versioning avec retour arrière** : le prompt système global et les fragments de persona sont tous les deux versionnés — chaque modification journalise la valeur remplacée (`PromptVersion`, jamais la valeur courante dupliquée), et restaurer une ancienne version passe par le même chemin d'édition normal (donc journalise à son tour la valeur courante — un "redo" reste possible après un retour arrière). `[Q53]`
